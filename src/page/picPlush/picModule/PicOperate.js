...
export default memo((props) => {
    const [picState, pic, setPutSrc] = useContext(PicRoute);
    const [color, setColor] = useState('#000000'); // 当前选中 颜色
    const [progress, setProgress] = useState([1,2]); // 滚动条 亮度 饱和度值
    const [scopeData, setScopeData] = useState(0.02); // 改色范围
    ...
    const subminBut = (opacity = 1) => {
        ...
        changePic(picState, pic, [arr1, arr2], hslToRgb(hslR, hslG, hslB), opacity, progress[0])
